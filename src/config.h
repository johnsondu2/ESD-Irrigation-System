...
const int SPINACH_SENSOR_PIN = 34;  //
...
const int SPINACH_VALVE_PINS[NUM_MAX_VALVES] = {14, -1, -1, -1, -1, -1, -1, -1, -1, -1};  // only 3 valves for spinach
...
enum Beds {
  SPINACH,
  NUM_BEDS  // SPINACH, LEEK, SPRING_ONION, GARLIC, PARSLEY, NUM_BEDS
};

// Bed class variables
/* NOTE: none of these values are official and they are all subject to testing */
const int MOISTURE_LOWER_THRESHOLD = 25;      /* 30%; needs to be tested */
const int MOISTURE_UPPER_THRESHOLD = 50;      /* 70%; needs to be tested */
const unsigned long WATERING_INTERVAL = 5000; /* 20 minutes; needs to be tested */
const unsigned long WAITING_INTERVAL = 5000;  /* 20 minutes; needs to be tested */
const int SENSOR_LOWER_BOUND = 0;             /* this value will depend on the sensor chosen */
const int SENSOR_UPPER_BOUND = 4095;          /* this value will depend on the sensor chosen */
